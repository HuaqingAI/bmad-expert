...
export async function update(options = {}) {
  ...
  try {
    ...
    printProgress('', true)
    initUpdateResult = { skipped: false, filesUpdated: 0, filesSkipped: 0, fileResults: [], error: err.message }
  }

  // ── Step 7: 成功输出（FR72: 逐文件摘要）────────────────────────────────
  const parts = [`已更新至 v${version}，用户配置和 memory 完整保留。`]
  if (initUpdateResult.fileResults && initUpdateResult.fileResults.length > 0) {
    parts.push(initUpdateResult.fileResults.map((f) => `${f.path}: ${f.action}`).join('\n'))
  }
 ...
 * @returns {Promise<{skipped: boolean, filesUpdated: number, filesSkipped: number, fileResults: Array<{path: string, action: string}>}>}
 */
export async function updateInitConfigs({ yes = false, force = false, cwd, currentVersion }) {
  ...
  if (!manifestExists) {
    return { skipped: true, filesUpdated: 0, filesSkipped: 0, fileResults: [] }
  }
  ...
  for (const fileEntry of manifest.files) {
    ...
    try {
      ...
      generationErrors++
      filesSkipped++
      fileResults.push({ path: fileEntry.path, action: 'skipped (error)' })
      continue
    }
    ...
    if (sectionId) {
      // 标记管理文件：精准段落替换（FR67）
      const action = await updateMarkerManagedFile({
        filePath,
        fileEntry,
        currentContent,
        newContent,
      ...
      fileResults.push({ path: fileEntry.path, action })
      if (action === 'section-replaced') {
        filesUpdated++
      } else {
        filesSkipped++
      }
      ...
      const action = await updateFullReplaceFile({
        filePath,
        fileEntry,
        currentContent,
        newContent,
  ...
  return { skipped: false, filesUpdated, filesSkipped, fileResults }
}
 ...
 * 标记外用户自定义内容不受影响。
 *
 * @returns {Promise<'section-replaced'|'skipped (no change)'|'skipped (no marker)'>}
 */
 ...
 *
 * @returns {Promise<'confirmed-overwrite'|'skipped (no change)'|'skipped (user declined)'>}
 */
