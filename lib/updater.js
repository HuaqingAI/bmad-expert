...
export async function update(options = {}) {
  const { platform: platformOverride = null, agentId = 'bmad-expert', yes = false, cwd } = options
  ...
  try {
    initUpdateResult = await updateInitConfigs({ yes, cwd, currentVersion: version })
  } catch (err) {
...
/**
 * 更新 init 生成的配置文件（FR58-FR60）
 *
 * 读取 .bmad-init.json 清单，对每个文件与新版模板比较差异，
 * 有差异时备份并展示变更摘要，等待用户确认后写入。
 *
 * @param {Object} params
 ...
 */
export async function updateInitConfigs({ yes = false, cwd, currentVersion }) {
  ...
  // ── 版本比较 ──
  if (manifest.templateVersion === currentVersion) {
