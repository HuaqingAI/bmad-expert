...
/**
 * Map manifest entry type to section-manager sectionId.
 * Only marker-managed file types have a sectionId.
 */
 ...
 * @returns {Promise<{toDelete: string[], toRemoveSection: Array<{path: string, sectionId: string, absolutePath: string}>, toPreserve: string[], hasManifest: boolean, hasBmadDir: boolean}>}
 */
export async function collectUninstallTargets(cwd, installPath, frameworkFiles, userDataPaths) {
  const toDelete = []
  const toRemoveSection = []
  const toPreserve = []
  ...
  if (manifest && Array.isArray(manifest.files)) {
    for (const entry of manifest.files) {
      ...
      // action === 'appended' → precision removal via section-manager
      // action === 'created' or missing (Phase 3 backward compat) → delete whole file
      const sectionId = TYPE_TO_SECTION_ID[entry.type]
      if (entry.action === 'appended' && sectionId) {
        toRemoveSection.push({ path: entry.path, sectionId, absolutePath: filePath })
      } else {
  ...
  return { toDelete, toRemoveSection, toPreserve, hasManifest, hasBmadDir }
}

/**
 * Display the cleanup plan via output.js.
 *
 * @param {{toDelete: string[], toRemoveSection?: Array<{path: string, sectionId: string, absolutePath: string}>, toPreserve: string[]}} plan
 */
export function displayCleanupPlan(plan) {
  ...
  if (plan.toRemoveSection && plan.toRemoveSection.length > 0) {
    lines.push('')
    lines.push('将移除 bmad 段落：')
    for (const item of plan.toRemoveSection) {
      lines.push(`  - ${item.absolutePath}`)
    }
  }
 ...
 *
 * @param {{toDelete: string[], toRemoveSection?: Array<{absolutePath: string}>}} plan
 * @param {string} cwd - Workspace root directory
...
export async function backupFiles(plan, cwd) {
  ...
  // Collect all file paths that will be affected
  const allPaths = [
    ...plan.toDelete,
    ...(plan.toRemoveSection || []).map((item) => item.absolutePath),
  ]
...
/**
 * Execute the uninstall: delete whole files, precision-remove marked sections, then remove manifest.
 * Uses per-item error collection to maximize cleanup even on partial failures.
 *
 * @param {{toDelete: string[], toRemoveSection?: Array<{path: string, sectionId: string, absolutePath: string}>, hasManifest: boolean}} plan
 * @param {string} cwd - Workspace root directory
...
export async function executeUninstall(plan, cwd) {
  ...
  // Phase 2: Precision remove marked sections (appended files)
  if (plan.toRemoveSection) {
    for (const item of plan.toRemoveSection) {
      try {
        if (await fs.pathExists(item.absolutePath)) {
          const content = await fs.readFile(item.absolutePath, 'utf8')
          const cleaned = removeBmadSection(content, item.sectionId)
          if (cleaned === content) {
            // Markers not found — nothing to remove, skip
            continue
          }
          if (cleaned.trim() === '') {
        ...
        errors.push({ path: item.absolutePath, err })
      }
    }
  }
...
export async function uninstall(options = {}) {
  ...
  if (plan.toDelete.length === 0 && plan.toRemoveSection.length === 0 && !plan.hasManifest && !plan.hasBmadDir) {
