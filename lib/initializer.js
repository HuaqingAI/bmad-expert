// lib/initializer.js
// init 命令核心逻辑 — Story 10.2 + 10.3（幂等保护）+ 10.4（智能追加）
//
// 架构约束（后续故事必须遵守）：
...
import { extractBmadSection, hasBmadSection } from './section-manager.js'
...
export async function resolveConflicts(cwd, fileChecks, options = {}) {
  ...
  for (const file of fileChecks) {
    ...
    if (options.yes) {
      if (file.type === 'workspace-claude' || file.type === 'project-claude') {
        const sectionId = file.type === 'workspace-claude' ? 'bmad-workspace-config' : 'bmad-project-config'
        const existingContent = await fs.readFile(join(cwd, file.path), 'utf8')

        if (hasBmadSection(existingContent, sectionId)) {
          results.push({ path: file.path, type: file.type, content: file.content, action: 'skipped' })
        } else {
          const bmadSection = extractBmadSection(file.content, sectionId)
          const separator = existingContent.endsWith('\n\n') ? '' : existingContent.endsWith('\n') ? '\n' : '\n\n'
          const appendedContent = existingContent + separator + bmadSection + '\n'
          results.push({ path: file.path, type: file.type, content: appendedContent, action: 'appended' })
        }
      } else {
        results.push({ path: file.path, type: file.type, content: file.content, action: 'skipped' })
      }
    ...
    if (file.type === 'workspace-claude' || file.type === 'project-claude') {
      ...
      if (hasBmadSection(existingContent, sectionId)) {
        ...
        answer = await askQuestion(
          `文件已存在: ${file.path}（未包含 BMAD 配置）\n  1. 追加 BMAD 配置段落（推荐） 2. 覆盖（先备份） 3. 跳过 4. 查看 diff\n请选择 (1/2/3/4): `
        )
        if (answer === '1') {
          const bmadSection = extractBmadSection(file.content, sectionId)
          const separator = existingContent.endsWith('\n\n') ? '' : existingContent.endsWith('\n') ? '\n' : '\n\n'
        ...
        // Remap: 2→1(overwrite), 3→2(skip), 4→3(diff) for standard handling below
...
export async function generateFiles(cwd, projectInfo, options = {}) {
  ...
  try {
    ...
    // ── Phase 1: Build content in memory (using reusable generateFileContent) ──
...
export async function writeManifest(cwd, files, defaultProject) {
  ...
  try {
    ...
    let manifest
    if (existingManifest) {
      // Merge: keep skipped files from old manifest, update overwritten/created/appended
      const existingFileMap = new Map(
        (existingManifest.files || []).map((f) => [f.path, f])
      )
      for (const rec of fileRecords) {
        if (rec.action === 'skipped') continue // preserve existing record
        existingFileMap.set(rec.path, { path: rec.path, type: rec.type, action: rec.action })
      }
      manifest = {
        ...existingManifest,
        updatedAt: now,
      ...
      manifest = {
        ...
        templateVersion: '1.0.0',
        defaultProject,
        files: fileRecords.map(({ path, type, action }) => ({ path, type, action })),
      }
    }
 ...
 *
 * @param {{yes?: boolean, cwd?: string, project?: string|null}} options
...
export async function init(options = {}) {
  ...
  if (options.project) {
    // --project <name>: skip detection and interactive selection (FR71)
    projectInfo = {
    ...
    projectInfo = await collectProjectInfo(workspaceInfo, { yes: options.yes })
  }
  ...
  // ── Summary（FR72: 逐文件摘要）──
  const summary = files.map((f) => `${f.path}: ${f.action}`).join('\n')
