...
5. **AC5 — AGENTS.md Workspace 环境检查（agent 模板层）**
   `agent/AGENTS.md` Session Startup 新增 Step 2.5：扫描 cwd 及一级子目录 BMAD 项目 → 检查 CLAUDE.md bmad 标记 → 缺失则 `npx bmad-expert init --yes` → 失败不阻塞
...
8. **AC8 — bmad-project-init.md install 后串联（agent 模板层）**
   install 成功后自动执行 `npx bmad-expert init --yes`，失败不阻塞
