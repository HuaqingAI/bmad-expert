...
- [x] 创建 `.github/workflows/ci.yml` (AC: #1)
  - [x] 触发条件：`push` 到 `main` 分支，`pull_request` 目标为 `main`
  - [x] Node.js 矩阵：`[20.19.x, 22.x]`
  - [x] 安装步骤：`npm ci`（不用 `npm install`）
  - [x] 测试步骤：`npm test`
  - [x] 失败时自动标记流水线为 failed

- [x] 创建 `.github/workflows/publish.yml` (AC: #2)
  - [x] 触发条件：`push` 到匹配 `v*.*.*` 的 tags
  - [x] 安装步骤：`npm ci`
  - [x] 发布步骤：判断是否为 prerelease tag（含 `-`）
    - 正式版（无 `-`）：`npm publish`
    - 预发版（含 `-`）：`npm publish --tag next`
  - [x] 配置 `NODE_AUTH_TOKEN` secret 用于 npm 认证
  - [x] 配置 npm registry URL：`https://registry.npmjs.org/`

- [x] 更新 `README.md` (AC: #3)
  - [x] 在文件顶部添加 npm publish 徽章占位（使用 `bmad-expert` 包名）
  - [x] 添加包名注册说明区块，提示需提前确认 `bmad-expert` 包名可用性

- [x] 在 `package.json` 中添加 `publishConfig`（如不存在）
  - [x] 确认 `"publishConfig": { "registry": "https://registry.npmjs.org/", "access": "public" }` 存在
...
- 全项目 lint 存在预存错误（lib/installer.js `options` unused var），为 Story 1.1 占位文件引入，非本 Story 造成。Story 1.4 相关文件（YAML/JSON/Markdown）零 lint 错误。

### Completion Notes List

- ✅ AC#1: `.github/workflows/ci.yml` 已创建，push/PR 触发，npm ci + npm test，Node 矩阵 20.19.x & 22.x
- ✅ AC#2: `.github/workflows/publish.yml` 已创建，`v*.*.*` tag 触发，prerelease 判断通过 `contains(github.ref_name, '-')` 实现，正式版 `npm publish`，预发版 `npm publish --tag next`
- ✅ AC#3: README.md 已添加 npm 版本徽章 + CI 徽章 + 包名注册说明区块
- ✅ package.json 已添加 `publishConfig`（registry + access: public）
- ✅ npm test: 33/33 通过，无回归

### File List

- .github/workflows/ci.yml（新建：CI 流水线，push main 触发，Node 矩阵测试）
- .github/workflows/publish.yml（新建：CD 发布流水线，v*.*.* tag 触发，prerelease 支持）
- README.md（修改：添加 npm 徽章 + 包名注册说明）
- package.json（修改：添加 publishConfig 字段）
- _bmad-output/implementation-artifacts/sprint-status.yaml（修改：状态 ready-for-dev → in-progress）
