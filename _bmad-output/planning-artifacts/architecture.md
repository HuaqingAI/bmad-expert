...
- 复杂度级别：中等
- 估计架构组件数：12-14 个（CLI 层、平台检测、适配器×4、安装编排器、智能参数构建、输出格式化、错误结构化、模板引擎、初始化器、卸载器）
...
```json
{
  ...
  "files": [
    ...
    { "path": "bmad-expert/CLAUDE.md", "type": "project-claude" },
    { "path": "bmad-expert/workflow/story-dev-workflow-single-repo.md", "type": "workflow" }
  ]
}
```

此清单供 `update` 和 `uninstall` 使用，判断哪些文件由 init 生成。

### 配置文件更新架构（Phase 3）

**扩展 `lib/updater.js`：**

现有 updater.js 更新 _bmad 框架文件。Phase 3 扩展其职责，增加 init 生成文件的更新能力。
...
3. 对清单中每个文件：
   a. 读取当前文件内容
   b. 读取对应的新版本模板 + 清单中记录的项目信息重新生成
   c. 比较差异
   d. 无差异 → 跳过
   e. 有差异 → 备份当前文件为 .bak，展示变更摘要，等待用户确认
4. 更新 .bmad-init.json 中的 templateVersion
```

**冲突处理策略（FR60）：**
- 不尝试自动合并——配置文件内容简短，用户手动处理更可靠
- 备份文件命名：`{filename}.bak.{timestamp}`
- `--yes` 模式下：备份 + 覆盖，不跳过（与 init 的 --yes 行为不同，因为用户已明确执行 update）
...
| 来源 | 清理目标 | 判断依据 |
|------|---------|---------|
| install | `_bmad/` 目录 | 目录存在性检测 |
| install | agent 补充文件（SOUL.md 等） | 平台适配器的 `getInstallPath()` |
| init | CLAUDE.md、workflow 文件 | `.bmad-init.json` 清单 |
| init | `.bmad-init.json` 自身 | 固定位置 |

**安全机制（FR62）：**
- 执行前展示完整清理计划（列出所有将删除的文件和目录）
...
npx bmad-expert uninstall
  ...
  → uninstaller.js（执行文件删除，跳过用户数据文件）
...
- FR51-FR57（工作环境初始化）：initializer.js + templates/ 覆盖
- FR58-FR60（配置文件更新）：updater.js 扩展覆盖
- FR61-FR63（卸载）：uninstaller.js 覆盖
