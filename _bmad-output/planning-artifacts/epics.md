...
- 【技术栈】Node.js 24 LTS（目标），最低兼容 Node.js 18+；纯 JavaScript ESM（`"type": "module"`）
...
**Then** 流水线触发，使用 `npm ci` 安装依赖，运行 `npm test`，Node.js 版本使用 18.x 和 22.x 矩阵
...
**Given** 当前 Node.js 版本低于 18
**When** CLI 启动时版本检测失败
**Then** stderr 输出 `ERROR [E003] 依赖缺失: Node.js 版本不足（当前 vX.X.X，需要 ≥18.0.0）\n修复步骤：\n  1. 升级 Node.js 至 18.x 或更高版本\n可重试：否`
