---
stepsCompleted: ['step-01-validate-prerequisites', 'step-02-design-epics', 'step-03-create-stories', 'step-04-final-validation', 'phase2-step-01-validate-prerequisites', 'phase2-step-02-design-epics', 'phase2-step-03-create-stories', 'phase2-step-04-final-validation', 'phase3-step-02-design-epics', 'phase3-step-03-create-stories', 'phase4-step-02-design-epics', 'phase4-step-03-create-stories']
inputDocuments:
  - '_bmad-output/planning-artifacts/prd.md'
  - '_bmad-output/planning-artifacts/architecture.md'
lastUpdated: '2026-04-09'
updateNote: 'Phase 4 增量追加：Epic 12（标记化精准管理引擎，Stories 12.1-12.4）、Epic 13（交互确认与 CLI 增强，Stories 13.1-13.2）；FR64-FR75 清单与 Coverage Map；Phase 4 架构约束'
---
...
FR51: 系统提供规范化 workspace 级 CLAUDE.md 模板，包含默认项目路由和全局约定声明（Phase 3）
FR52: 系统提供规范化 project 级 CLAUDE.md 模板，包含工作流触发词定义和项目编码规范引用（Phase 3）
FR53: 系统提供规范化 workflow 模板，仅描述步骤意图，不绑定特定技术栈或构建工具，由执行 AI 根据项目上下文决定具体命令（Phase 3）
FR54: 系统提供 `init` 命令，检测 workspace 目录结构并交互式收集项目信息（Phase 3）
FR55: `init` 命令根据模板和用户输入在 workspace 根目录生成 CLAUDE.md，在项目目录生成 project CLAUDE.md 和 workflow 文件（Phase 3）
FR56: `init` 命令在检测到已有配置文件时提供覆盖、跳过或合并选项，不静默破坏用户已有配置（Phase 3）
FR57: `init` 命令与 `install` 命令独立运作，可单独执行也可串联执行（Phase 3）
FR58: `update` 命令在更新 BMAD 方法论文件的同时，检测 init 生成的配置文件与新版本模板的差异（Phase 3）
FR59: 配置文件更新前自动备份用户当前文件，更新后展示变更摘要供用户确认（Phase 3）
FR60: 用户已定制的内容在更新过程中不被覆盖（Phase 3）
FR61: 系统提供 `uninstall` 命令，清理 init 生成的配置文件和 install 安装的 _bmad 目录（Phase 3）
FR62: `uninstall` 执行前要求用户确认，并提供备份选项（Phase 3）
FR63: `uninstall` 完成后输出清理结果摘要，明确列出已删除和已保留的文件（Phase 3）

**Phase 4 新增需求：**

FR64: 系统提供标记化段落管理引擎，支持通过 HTML 注释开闭标记对（`<!-- bmad-xxx -->` / `<!-- /bmad-xxx -->`）对配置文件中的管理段落执行精准替换操作（Phase 4）
FR65: 系统支持通过标记定位精准移除配置文件中的管理段落，移除后保留文件中标记外的用户自定义内容不变（Phase 4）
FR66: project 级 CLAUDE.md 模板使用 `<!-- bmad-project-config -->` / `<!-- /bmad-project-config -->` 标记包裹 bmad 管理段落，与 workspace CLAUDE.md 的标记化管理保持一致（Phase 4）
FR67: `update` 命令对标记管理文件（workspace CLAUDE.md、project CLAUDE.md）执行基于标记的精准段落替换，仅更新标记内的自动生成内容，不影响用户在标记外添加的自定义内容（Phase 4）
FR68: `update` 命令对框架文件（SOUL.md、IDENTITY.md、AGENTS.md、BOOTSTRAP.md）更新前向用户展示变更摘要并等待确认，不再静默覆盖（Phase 4）
FR69: `update` 命令对框架文件更新前自动创建备份（`{filename}.bak.{timestamp}`），确认后才执行覆盖（Phase 4）
FR70: `uninstall` 命令对 `.bmad-init.json` 中 `action: "appended"` 的文件仅移除标记段落而非删除整个文件，保留用户原有内容（Phase 4）
FR71: `init` 命令支持 `--project <name>` 参数，在多项目 workspace 中定向初始化指定项目而无需交互选择（Phase 4）
FR72: `init --yes` 和 `update --yes` 模式执行后输出完整操作摘要，列出每个文件的处理结果（created/appended/skipped/replaced/confirmed），使 AI caller 可解析执行效果（Phase 4）
FR73: `status` 命令输出扩展显示 init 状态信息（是否执行过 init、模板版本、配置文件完整性），使 AI caller 可判断是否需要执行 init（Phase 4）
FR74: `update` 命令支持 `--force` 参数，跳过版本门控（templateVersion === currentVersion 时仍执行更新流程），适用于用户想重置模板内容或开发调试场景（Phase 4）
FR75: `init` 和 `update` 命令在交互模式下对已有文件提供明确的操作选项菜单（追加/覆盖/跳过/查看 diff），默认选项根据文件类型智能推荐（标记管理文件推荐追加，框架文件推荐确认后覆盖）（Phase 4）
...
- 【多平台自动检测探针链】检测顺序：--platform 显式参数 → HappyCapy 探针（~/.happycapy/ 或 HAPPYCAPY_* 环境变量）→ OpenClaw 探针 → 全未命中则提示手动指定（Claude Code / Codex 探针延期，待适配器架构重新设计后纳入）
...
- 【模板体系】模板文件存储于 npm 包 `templates/` 目录：workspace-claude.md、project-claude.md、workflow-single-repo.md。纯内容模板，无占位符变量——执行 AI 根据项目上下文自行决定具体命令。模板内容需从规范角度重新设计，不可直接照搬现有项目文件
- 【init 命令 initializer.js】新增模块：workspace 结构检测（扫描子目录识别项目目录）、交互式信息收集（项目名、路径、工作流偏好）、文件生成（模板 + 项目信息 → 最终文件）；不使用模板引擎库，原生字符串处理
- 【.bmad-init.json 清单】init 完成后在 workspace 根生成清单文件，记录生成的文件路径和模板版本；供 update 和 uninstall 读取，判断哪些文件由 init 生成
- 【配置文件更新策略】扩展 updater.js：读取 .bmad-init.json → 对比新版模板差异 → 备份当前文件 → 展示变更摘要 → 用户确认后写入。不尝试自动合并——配置文件短小，用户手动处理更可靠
- 【uninstall 命令 uninstaller.js】新增模块：清单驱动清理（.bmad-init.json + _bmad/ + 适配器路径）；不删除用户数据文件（MEMORY.md、USER.md、memory/）；新增 exit code 7（NOT_INSTALLED）
- 【init --yes 行为】跳过已有文件不覆盖（安全默认）；与 update --yes 不同（备份后覆盖）

**Phase 4 新增架构约束：**

- 【标记化段落管理引擎 section-manager.js】新增模块：统一管理所有标记段落的 CRUD 操作（hasBmadSection、extractBmadSection、replaceBmadSection、removeBmadSection、wrapBmadSection）；纯字符串操作（indexOf + substring），不使用正则；残缺标记视为无效
- 【文件三分类契约】框架文件（SOUL.md 等）→ 确认+备份+覆盖；标记管理文件（CLAUDE.md）→ 精准段落替换/移除；用户数据（MEMORY.md 等）→ 永不接触。三类文件定义在 package.json 的 bmadExpert.fileCategories 中
- 【.bmad-init.json action 字段】记录每个文件的写入方式（`created` / `appended`），决定 uninstall 行为：created → 删除整文件；appended → 仅移除标记段落
- 【update 精准替换】标记管理文件调用 section-manager.replaceBmadSection() 替换标记段落，用户自定义内容不变；框架文件需交互确认后覆盖
- 【uninstall 精准移除】对 action: "appended" 的文件调用 section-manager.removeBmadSection() 仅移除标记段落；移除后文件为空则删除
- 【project-claude 标记化】templates/project-claude.md 使用 `<!-- bmad-project-config -->` / `<!-- /bmad-project-config -->` 标记包裹管理段落
- 【extractBmadSection 迁移】Phase 3 initializer.js 中的 extractBmadSection() 迁移至 section-manager.js，initializer.js 改为引用
...
FR47: Epic 8 - 自动检测宿主平台，无需 `--platform`；Phase 2 范围：OpenClaw 平台（Claude Code 延期）
FR48: 【延期】Codex 平台环境检测、写入路径、注册契约（Claude Code / Codex 不支持当前适配器架构，待重新设计）
FR49: Epic 9 - `status --json` 完整结构化输出
FR50: Epic 9 - README 全命令覆盖

**Phase 3 FR Coverage：**

FR51: Epic 10 - 规范化 workspace 级 CLAUDE.md 模板
FR52: Epic 10 - 规范化 project 级 CLAUDE.md 模板
FR53: Epic 10 - 规范化 workflow 模板（步骤意图，不绑技术栈）
FR54: Epic 10 - `init` 命令检测 workspace 目录结构并交互收集项目信息
FR55: Epic 10 - `init` 命令生成 workspace CLAUDE.md、project CLAUDE.md 和 workflow 文件
FR56: Epic 10 - `init` 命令检测已有文件时提供覆盖/跳过/合并选项（Story 10.3 实现跳过/覆盖，Story 10.4 补充追加语义）
FR57: Epic 10 - `init` 与 `install` 独立运作可串联
FR58: Epic 11 - `update` 扩展检测 init 配置文件与新版本差异
FR59: Epic 11 - 配置文件更新前备份，更新后展示变更摘要
FR60: Epic 11 - 用户已定制内容在更新中不被覆盖
FR61: Epic 11 - `uninstall` 命令清理 init + install 产物
FR62: Epic 11 - `uninstall` 执行前确认并提供备份选项
FR63: Epic 11 - `uninstall` 完成后输出清理结果摘要

**Phase 4 FR Coverage：**

FR64: Epic 12 - 标记化段落管理引擎（替换操作）
FR65: Epic 12 - 标记化段落精准移除
FR66: Epic 12 - project CLAUDE.md 标记化
FR67: Epic 12 - update 基于标记的精准段落替换
FR68: Epic 12 - update 框架文件确认机制
FR69: Epic 12 - update 框架文件备份机制
FR70: Epic 12 - uninstall 对 appended 文件精准段落移除
FR71: Epic 13 - init --project 参数
FR72: Epic 13 - --yes 模式输出完整操作摘要
FR73: Epic 13 - status 扩展显示 init 状态
FR74: Epic 13 - update --force 跳过版本门控
FR75: Epic 13 - init/update 交互模式智能选项菜单
...
### Epic 8：多平台扩展 — OpenClaw 平台扩展（Phase 2）

无需指定 `--platform` 参数，系统通过探针链自动识别宿主平台；OpenClaw 平台用户可通过相同命令完成 BMAD 安装，获得与 HappyCapy 一致的体验。Claude Code 和 Codex 平台因架构差异延期，待重新设计后纳入后续 Phase。（依赖 Epic 7 的编排架构）
**FRs covered:** FR47（范围：OpenClaw 平台自动检测）
**NFRs covered:** NFR5（2 平台：HappyCapy + OpenClaw）、NFR7、NFR8、NFR15
...
### Epic 10：模板体系与 init 命令 — 工作环境初始化（Phase 3）

用户执行 `npx bmad-expert init` 后，系统检测 workspace 结构、交互式收集项目信息，根据规范化模板生成 workspace CLAUDE.md、project CLAUDE.md 和 workflow 文件。新会话启动时自动路由到正确项目，工作流触发词直接可用。从"装好 BMAD"到"完整工作环境"的最后一步被填平。
**FRs covered:** FR51、FR52、FR53、FR54、FR55、FR56、FR57
**Stories:** 10.1（模板设计）、10.2（init 核心逻辑）、10.3（幂等保护）、10.4（Agent Onboarding 串联与 init --yes 追加修复）

### Epic 11：配置文件更新与卸载命令（Phase 3）

`update` 命令在更新 BMAD 方法论文件的同时检测 init 生成的配置文件版本差异，备份后展示变更摘要供用户确认；`uninstall` 命令清理 init + install 全部产物，保护用户数据不被误删。（依赖 Epic 10 的 init 命令和 .bmad-init.json 清单）
**FRs covered:** FR58、FR59、FR60、FR61、FR62、FR63

### Epic 12：标记化精准管理引擎与操作升级（Phase 4）

标记化段落管理引擎（section-manager.js）作为基础设施，支撑 update 和 uninstall 从"全文件操作"升级为"精准段落操作"：update 对标记管理文件只替换标记内段落（用户自定义内容不受影响），对框架文件增加确认+备份机制；uninstall 对 appended 文件只移除标记段落而非删除整文件；project CLAUDE.md 也纳入标记化管理体系。（依赖 Epic 10/11 的 init + update + uninstall 基础实现）
**FRs covered:** FR64、FR65、FR66、FR67、FR68、FR69、FR70
**Stories:** 12.1（section-manager 核心模块）、12.2（init 增强 — project-claude 标记化 + action 字段）、12.3（update 精准段落替换 + 框架文件确认）、12.4（uninstall 精准段落移除）

### Epic 13：交互确认与 CLI 增强（Phase 4）

CLI 命令增加精细化控制能力：`--project` 支持多项目定向、`--force` 跳过版本门控、`--yes` 输出完整操作摘要供 AI 解析、`status` 扩展显示 init 状态、交互模式提供智能推荐的操作选项菜单。面向 AI caller 和 CLI 用户两个受众同时提升操控粒度。（依赖 Epic 12 的 section-manager 和精准操作基础）
**FRs covered:** FR71、FR72、FR73、FR74、FR75
**Stories:** 13.1（--project + --force 参数）、13.2（--yes 输出增强 + 交互确认菜单）、13.3（status init 状态扩展）
...
> **延期说明（2026-04-07）：** Story 8-3（Claude Code 适配器）和 Story 8-4（Codex 适配器）已取消。原因：Claude Code 和 Codex 平台不原生支持本项目的 agent 注册架构（adapter.detect() + adapter.install() + 文件写入路径 + 注册契约模型），通常需通过 skills + 项目配置文件（CLAUDE.md/AGENTS.md）或 plugins 机制实现。最优实现方案待确定后重新设计。
...
### ~~Story 8.3：Claude Code 平台适配器~~ 【已取消 — 2026-04-07】

> **延期原因：** Claude Code 不原生支持 agent 注册架构，通常需通过 skills + 项目配置文件（CLAUDE.md/AGENTS.md）实现，与当前适配器接口设计不兼容，最优实现方案待确定后重新设计。

### ~~Story 8.4：Codex 平台适配器~~ 【已取消 — 2026-04-07】

> **延期原因：** Codex 不原生支持 agent 注册架构，通常需通过 plugins 机制实现，与当前适配器接口设计不兼容，最优实现方案待确定后重新设计。
...
> **范围调整说明（2026-04-07）：** Phase 2 README 仅覆盖 HappyCapy + OpenClaw 安装命令。Claude Code / Codex 适配器延期，README 如实反映当前支持状态。
...
**When** 审阅模板内容
**Then** 包含以下规范化章节：① 默认项目声明（占位，init 时填入实际项目名和路径）；② 全局约定（代码仓库操作入口、skill 路由声明）
**And** 内容仅描述结构和意图，不包含特定项目的硬编码信息
**And** 模板可适用于不同技术栈的项目（Java/Node/Python 等）

**Given** `templates/project-claude.md` 已设计
**When** 审阅模板内容
**Then** 包含以下规范化章节：① 工作流触发词定义区块（start story 等触发词及其对应的 workflow 文件引用）；② 项目规范引用区块（编码规范文档路径占位）
**And** 触发词定义与 workflow 模板中的步骤对应，无悬空引用

**Given** `templates/workflow-single-repo.md` 已设计
**When** 审阅模板内容
**Then** 包含通用 story 开发工作流步骤序列：同步代码 & 创建分支 → 创建 Story 文件 → 实现 Story → 编译验证（可选）→ 运行测试（可选）→ 代码审查（可选）→ 提交代码 → 推送 & 创建 PR → 更新 Sprint 状态 → 合并 & 清理
...
**When** 执行 `npx bmad-expert init`
**Then** 对每个已有文件交互提示三选一：① 覆盖（先备份为 `CLAUDE.md.bak.{timestamp}`）；② 跳过（保留原文件不动）；③ 查看 diff（展示当前文件与新模板的差异，再决定覆盖或跳过）（FR56）
...
**背景：** Sprint Change Proposal 2026-04-09。Phase 3 功能已发版但 agent 层未串联 CLI 的 init 命令，导致流程断裂。HappyCapy 等平台 workspace 默认带 CLAUDE.md，init --yes 直接 skip 导致初始化实质失效。

**Acceptance Criteria:**

**AC1 — init --yes 智能追加（代码层）**

**Given** workspace 根目录已存在 CLAUDE.md 但不含 `<!-- bmad-workspace-config -->` 标记（如平台默认生成的 CLAUDE.md）
**When** 执行 `npx bmad-expert init --yes`
**Then** 从模板提取 bmad 配置段落（`<!-- bmad-workspace-config -->` 至 `<!-- /bmad-workspace-config -->` 之间的内容），追加到已有文件末尾，原有内容完整保留
**And** 文件处理结果标记为 `action: 'appended'`
**And** 追加前确保空行分隔，内容不粘连

**AC2 — init --yes 幂等（代码层）**

**Given** workspace CLAUDE.md 已包含完整的 bmad 配置标记对（开闭标记都在）
**When** 执行 `npx bmad-expert init --yes`
**Then** 该文件被 skip，不重复追加
**And** 残缺标记（只有开标记没有闭标记）视为无效，触发重新追加完整段落

**AC3 — 交互模式追加选项（代码层）**

**Given** workspace CLAUDE.md 已存在但不含 bmad 标记
**When** 执行 `npx bmad-expert init`（交互模式）
**Then** 提示增加"追加 BMAD 配置段落"选项（作为默认推荐），在原有"覆盖/跳过/查看 diff"之前

**AC4 — 模板标记（代码层）**

**Given** `templates/workspace-claude.md` 模板文件
**Then** bmad 管理的段落用 `<!-- bmad-workspace-config -->` / `<!-- /bmad-workspace-config -->` 开闭标记包裹
**And** 标记为 HTML 注释，不影响 Markdown 渲染

**AC5 — AGENTS.md Workspace 环境检查（agent 模板层）**

**Given** `agent/AGENTS.md` 模板的 Session Startup 段落
**Then** 新增 Step 2.5「Workspace 环境检查」，在每次 session 启动时执行：
  - 扫描 cwd 及一级子目录中的 BMAD 项目（`_bmad/bmm/config.yaml` 或 `_bmad/_config/manifest.yaml`）
  - 检查 cwd 下 CLAUDE.md 是否包含 `<!-- bmad-workspace-config -->` 标记
  - 标记缺失 → 在第一个 BMAD 项目目录下执行 `npx bmad-expert init --yes`
  - 操作幂等，已就绪的 workspace 秒过
  - init 失败不阻塞 session

**AC6 — AGENTS.md 手动刷新触发（agent 模板层）**

**Given** 用户说 "bmad workspace refresh"、"bmad reinit"、"刷新 bmad 环境"、"重新检测 workspace" 等
**When** agent 收到触发词
**Then** 重新执行 Step 2.5 的完整检查流程

**AC7 — BOOTSTRAP.md 职责分离（agent 模板层）**

**Given** `agent/BOOTSTRAP.md` 模板
**Then** 步骤 ③ 的 BMAD 环境检测逻辑改为引用 AGENTS.md Step 2.5（而非重复定义）
**And** BOOTSTRAP 仍在首次 session 中触发一次 workspace 检查，但逻辑定义在 AGENTS.md 中统一维护

**AC8 — bmad-project-init.md install 后串联（agent 模板层）**

**Given** `agent/bmad-project-init.md` install 流程
**Then** install 成功后（Step 3 verify 通过）自动执行 `npx bmad-expert init --yes`
**And** init 失败不阻塞，输出手动命令提示后继续进入 bmad-help

**AC9 — installer.js 输出提示（代码层）**

**Given** CLI install 成功后的输出信息
**Then** 追加提示：`如果你在使用 CLI，运行 npx bmad-expert init 配置工作环境。`
...
**When** 运行 `npm test`
**Then** 以下场景全部通过：
  - HappyCapy 默认 CLAUDE.md（无 bmad 标记）+ init --yes → append
  - CLAUDE.md 含完整标记 + init --yes → skip
  - 残缺标记 → 重新追加
  - 追加后内容空行分隔正确
  - 新增 `action: 'appended'` 状态在摘要输出中正确显示

**实施依赖：**
- AC1-AC4、AC9-AC10 为代码层变更，是 AC5-AC8（agent 模板层）正常工作的前置条件
- AC5-AC8 为 agent 模板层变更，可在代码层合并后并行实施

**参考：** `_bmad-output/planning-artifacts/sprint-change-proposal-2026-04-09.md`
...
**Then** 在完成 _bmad 框架文件更新后，对 init 清单中的每个文件：① 读取当前文件；② 用新版模板 + 清单中的项目信息重新生成；③ 比较差异；④ 无差异 → 跳过并提示；⑤ 有差异 → 备份当前文件为 `{filename}.bak.{timestamp}`，展示变更摘要，等待用户确认后写入（FR58、FR59）
...
**Then** 收集清理目标：① `.bmad-init.json` 清单中的所有文件；② `_bmad/` 目录；③ 平台适配器路径中的 agent 文件（FR61）
**And** 展示完整清理计划，列出所有将删除的文件和目录
**And** 明确标出将被**保留**的文件：MEMORY.md、USER.md、memory/ 目录
**And** 等待用户确认后执行删除（FR62）
**And** 删除完成后输出清理结果摘要：已删除 N 个文件、已保留 N 个用户数据文件（FR63）
**And** 最后删除 `.bmad-init.json` 自身
**And** 进程以 exit code 0 退出

**Given** 传入 `--backup` 参数
**When** 执行 uninstall
**Then** 删除前将所有目标文件复制到 `.bmad-backup-{timestamp}/` 目录（FR62）
...
标记化段落管理引擎（section-manager.js）作为基础设施，支撑 update 和 uninstall 从"全文件操作"升级为"精准段落操作"：update 对标记管理文件只替换标记内段落（用户自定义内容不受影响），对框架文件增加确认+备份机制；uninstall 对 appended 文件只移除标记段落而非删除整文件；project CLAUDE.md 也纳入标记化管理体系。
...
**Given** content 中包含完整标记段落
**When** 调用 `extractBmadSection(content, 'bmad-workspace-config')`
**Then** 返回标记段落完整内容（含开闭标记本身）；标记不存在时返回 `null`（FR64）

**Given** content 中包含完整标记段落，newSection 为新内容
**When** 调用 `replaceBmadSection(content, 'bmad-workspace-config', newSection)`
**Then** 标记内段落被 newSection 替换，标记外用户内容不变，替换后段落前后各有空行分隔（FR64）

**Given** content 中包含完整标记段落
**When** 调用 `removeBmadSection(content, 'bmad-workspace-config')`
**Then** 标记段落（含标记本身）被移除，标记外内容完整保留，多余空行被清理（连续 3+ 换行合并为 2 个）（FR65）

**Given** content 中不存在目标标记段落
**When** 调用 `removeBmadSection(content, 'bmad-workspace-config')`
**Then** 返回原内容不变（幂等）（FR65）

**Given** 调用 `wrapBmadSection(content, 'bmad-project-config')`
**When** 执行
**Then** 返回 `<!-- bmad-project-config -->\n{content}\n<!-- /bmad-project-config -->`（FR64）

**Given** Phase 3 `initializer.js` 中已有的 `extractBmadSection()` 函数
**When** 迁移至 section-manager.js
**Then** initializer.js 改为 `import { extractBmadSection } from './section-manager.js'`，功能不变
...
**When** 审阅内容
**Then** bmad 管理的段落用 `<!-- bmad-project-config -->` / `<!-- /bmad-project-config -->` 开闭标记包裹（FR66）

**Given** 执行 `npx bmad-expert init`，目标目录下不存在 project CLAUDE.md
**When** init 生成文件
**Then** 生成完整 project CLAUDE.md（含标记），`.bmad-init.json` 中该文件记录 `"action": "created"`（FR66）

**Given** 执行 `npx bmad-expert init --yes`，目标目录已存在 project CLAUDE.md 但不含 bmad 标记
**When** init 处理该文件
**Then** 从模板提取 `<!-- bmad-project-config -->` 段落追加到已有文件末尾，`.bmad-init.json` 中该文件记录 `"action": "appended"`（FR66）
...
**Then** 跳过项目检测和交互选择，直接使用 `my-app` 作为项目名和路径（FR71）
...
**When** 执行 `npx bmad-expert uninstall`
**Then** 调用 `section-manager.removeBmadSection()` 移除 `<!-- bmad-workspace-config -->` 段落，保留用户原有内容（FR70）
**And** 清理计划中该文件标注"移除 bmad 段落"而非"删除文件"

**Given** `.bmad-init.json` 中 project CLAUDE.md 记录 `"action": "appended"`
**When** 执行 uninstall
**Then** 同样仅移除 `<!-- bmad-project-config -->` 段落（FR70）
...
**Then** 视为 `"action": "created"`，删除整文件（向后兼容）
...
CLI 命令增加精细化控制能力：`--project` 支持多项目定向、`--force` 跳过版本门控、`--yes` 输出完整操作摘要供 AI 解析、`status` 扩展显示 init 状态、交互模式提供智能推荐的操作选项菜单。面向 AI caller 和 CLI 用户两个受众同时提升操控粒度。
...
**When** 完成后
**Then** 输出操作摘要，每个文件标注处理结果：`CLAUDE.md: appended`、`bmad-expert/CLAUDE.md: created`、`bmad-expert/workflow/...: created`（FR72）

**Given** 执行 `npx bmad-expert update --yes`，处理了框架文件和标记管理文件
**When** 完成后
**Then** 输出操作摘要：`CLAUDE.md: section-replaced`、`SOUL.md: confirmed-overwrite`、`IDENTITY.md: skipped (no change)`（FR72）

**Given** 执行 `npx bmad-expert init`（交互模式），workspace CLAUDE.md 已存在但无 bmad 标记
**When** 处理该文件
**Then** 选项菜单为：① 追加 BMAD 配置段落（推荐）② 覆盖（先备份）③ 跳过 ④ 查看 diff，默认推荐"追加"（FR75）

**Given** 执行 `npx bmad-expert init`（交互模式），workflow 文件已存在
**When** 处理该文件
**Then** 选项菜单为：① 覆盖（先备份）② 跳过 ③ 查看 diff，默认推荐"覆盖"（框架文件语义）（FR75）
